...
// Дизайн приближен к мобильному приложению (тёплый градиент, карточки-тайлы).
  ...
  return (
    <div>
      <style>{`
        ...
        .tl-cat:hover { transform: translateY(-3px); box-shadow: 0 12px 28px rgba(74,63,53,0.14); border-color: #B08D57; }
        .tl-cat:hover .tl-cat-ic { background: #B08D57; }
      ...
      <section style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg,#B08D57 0%,#9a7647 55%,#6B5A4D 100%)', color: '#fff', borderRadius: 24, padding: '30px 26px', marginBottom: 30, boxShadow: '0 16px 40px rgba(74,63,53,0.22)' }}>
        {/* декоративный орнамент-казан */}
        <div aria-hidden style={{ position: 'absolute', right: -10, top: -18, fontSize: 150, opacity: 0.12, lineHeight: 1, transform: 'rotate(-8deg)' }}>🍲</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: 16 }}>
        {CATEGORIES.map((c) => (
          <Link key={c.slug} href={`/app/catalog/${c.slug}`} className="tl-cat"
            ...
            <span className="tl-cat-ic" style={{ width: 60, height: 60, borderRadius: 18, background: '#F3EADB', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 30, transition: 'background .18s ease' }}>{c.icon}</span>
